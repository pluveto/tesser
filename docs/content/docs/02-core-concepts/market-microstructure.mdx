...
*   **Tesser Example**: The `PairsTrading` strategy implements this using cointegration concepts. Configure the two legs with fully-qualified identifiers (`exchange:SYMBOL`, e.g., `binance_perp:BTCUSDT`) so the engine knows which venue each leg belongs to.
