...
> ℹ️ Normalize fresh CSV downloads first via `tesser-cli data normalize --config ./configs/etl/<mapping>.toml` so the backtester receives canonical Parquet files.
